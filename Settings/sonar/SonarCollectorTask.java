...
public class SonarCollectorTask extends CollectorTask<SonarCollector> {
    ...
    public void collect(SonarCollector collector) {
        ...
        if (!CollectionUtils.isEmpty(collector.getSonarServers())) {
            ...
            for (int i = 0; i < collector.getSonarServers().size(); i++) {
                ...
                refreshData(enabledProjects(collector, projects), sonarClient, metrics);
    ...
    private List<SonarProject> enabledProjects(SonarCollector collector, List<SonarProject> projects) {
        List<SonarProject> enabledProjects = new ArrayList<>();
        SonarProject sonarProject;
        for (SonarProject project : projects) {
            sonarProject = sonarProjectRepository.findSonarProject(collector.getId(), project.getInstanceUrl(), project.getProjectId());
            if (!sonarProject.equals(null)) {
                enabledProjects.add(sonarProject);
            }
        }
